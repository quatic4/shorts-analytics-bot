...
import { getGuildConfig,setGuildConfig,getPublicSnapshot,setPublicSnapshot,deletePublicSnapshot,getDailySnapshot,setDailySnapshot } from "../lib/redis.js";
...
if(cmd==="uploadtest"){if(!hasManageGuild(i))return res.status(200).json(ephemeral("You need **Manage Server** permission."));const c=await getGuildConfig(guildId)||{};if(!c.uploadNotifyChannelId)return res.status(200).json(ephemeral("Upload notifier is not configured yet. Use **/uploadsetup** first."));await sendChannelMessage(c.uploadNotifyChannelId,{content:"@everyone 📢 **TEST UPLOAD ALERT**",embeds:[{title:"Upload notifier test",description:"If you can see this card, the Discord side of the upload notifier is working.",fields:[{name:"Discord delivery",value:"✅ Bot can post here\n✅ Embeds work\n✅ @everyone is being sent",inline:false},{name:"If real uploads still fail",value:"Then the problem is the YouTube webhook/subscription side, not Discord.",inline:false}],footer:{text:"GinduyahBOT • Upload Notifier Test"},timestamp:new Date().toISOString()}],allowed_mentions:{parse:["everyone"]}});return res.status(200).json(ephemeral(`✅ Test alert sent to <#${c.uploadNotifyChannelId}>.`))}
...
if(cmd==="today"){
  ...
  for(const t of tracked){
    try{
      ...
      if(!p){p=ch;await setDailySnapshot(guildId,t.channelId,ch);}
